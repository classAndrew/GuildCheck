const guildLink = "https://api.wynncraft.com/public_api.php?action=guildStats&command="

async function activityGuild(name = "Titans Valor") {
    let guild = await (await fetch(guildLink + name)).json()
    return Promise.all(guild.members.map((member) => fetch(`https://api.wynncraft.com/v2/player/${member.uuid}/stats`)))
        .then((res) => {
